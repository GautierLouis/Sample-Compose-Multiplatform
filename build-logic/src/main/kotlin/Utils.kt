...
internal fun Project.installBasePlugin() {
    with(pluginManager) {
        ...
        apply(libs.findPlugin("mokkery-plugin").get().get().pluginId)
    }
}
...
internal fun CommonExtension<*, *, *, *, *, *>.applySharedAndroidSettings(project: Project) {
    ...
    compileSdk = project.libs.findVersion("android-compile-sdk").get().requiredVersion.toInt()
    defaultConfig {
        minSdk = project.libs.findVersion("android-min-sdk").get().requiredVersion.toInt()
    }
    compileOptions {
